...
def rename(tuple_list):
	...
	return [(n['Name'],v[1]) for n,v in zip(record,tuple_list)]
...
class Subnetwork():
	...
	def print_net(self, output, significance = 0.6, filename = 'subnetwork.pk'):
		...
		ofp = open(output,'w')
		print >> ofp, 'score\thead_node\tall_nodes'
	...
	def rank_gene(self, data_mat, cutoff, output):
		...
		metric1 = rename(sorted_map(count_dict)[:1000]) # network count
		metric2 = rename(sorted_map(count2_dict)[:1000]) # weighted network count * importance
		ofp = open(output,'w')
		print >> ofp, 'm1_name\tm1_value\tm2_name\tm2_value'
		for a,b in zip(metric1,metric2):
			print >> ofp, str(a[0])+'\t'+str(a[1])+'\t'+str(b[0])+'\t'+str(b[1])
		ofp.close()
		return
